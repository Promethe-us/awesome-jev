...
> Community-maintained; not affiliated with TypeSafe AI. **Full source audit: 2026-09-24 (Asia/Shanghai).** This page distinguishes official statements, authors' experiments, and unverified leads. Being able to read a source does not mean this repository reproduced its performance claims. See the [source audit](docs/SOURCES_EN.md) for coverage and access limits.
...
| 09-23, Chinese aggregator index (added 09-24) | [DeepSleep, “GPT6 too slow? Try Jev” (Chinese)](https://www.xiaohongshu.com/explore/6aafe241000000001103375f?xsec_source=pc_feed&xsec_token=ABBYuHgvK74N6nwyt2XSUtiXIRfSD7DdEkRk9hEf7bpsE%3D) | Title, attribution, and the tokened link come from the [CodeAlex52/awesome-jev-cn](https://github.com/CodeAlex52/awesome-jev-cn) index; the tokened visit still returned “page not found,” text unverified |

> **About `xsec_token`:** Xiaohongshu in recent years added `xsec_token` (together with `xsec_source`) to note links as a security-check and anti-crawler parameter. A note link opens only when it carries a valid token bound to the session and source; without it, a direct visit hits a login wall, a “temporarily unavailable” notice, or a 404. Most bare note links in this section therefore fail when opened directly in a browser — **this does not mean the notes were deleted** — and tokened links also stop working once the token expires.
