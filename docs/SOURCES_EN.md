...
| The September 2026 paper wave (added 09-24) | The lead came from [PaperWeekly's September 23 roundup](https://mp.weixin.qq.com/s/kK3du8zji4fa_9chnBl7Dw); all 13 arXiv IDs were then checked one by one via the [arXiv API](https://export.arxiv.org/api/query?id_list=2609.22753,2609.23136,2609.24052,2609.24965,2609.23986,2609.26532,2609.26550,2609.23959,2609.24395,2609.25845,2609.23886,2609.24574,2609.26758) for title, authors, and first-submission date (13/13 matched, including exactly six first posted on 09-21). Content summaries and numbers come from the papers' abstracts. Five companion GitHub repositories had `full_name` and stars checked (09-24 snapshot); their code was not audited |
| 2026-09-24 community increment | **GitHub:** six repositories (jev-benchmarks, jev-arena, pi-warden, jev-codex-router, jev-hub, awesome-jev-cn) checked via the public API for `full_name` and stars; the READMEs of jev-benchmarks / jev-arena / jev-codex-router were read to verify figures and disclaimers. **Articles and docs:** five pages were readable over HTTP with key figures extracted — the Tencent Cloud ADP blog, Pydantic AI's docs, Pedaily (Kazik's authorized reprint, 09-21), Huxiu (Guixingren Pro reprint, 09-20), and Tencent Cloud Developer (programmer Yupi); the Guixingren Pro WeChat original returned a verification wall and the three Zhihu entries 403 on automated fetches — the owner confirmed on 09-24 that all three open in a browser, with key points to be added once read. **X:** three posts (@rinte0321, @ctatedev, @yanhua1010) checked via the FxTwitter mirror for author, UTC time, and text. **Xiaohongshu:** the DeepSleep note returned “page not found,” so it stays an unverified lead from a third-party index. Numbers from unreadable sources are never credited |
| 2026-09-24 full link audit | All 10 Markdown files checked link by link: **219 external URLs, 65 GitHub repositories**, plus every internal anchor and relative link. No GitHub 404s and no repository renames other than SemIf ([SemIf-OpenJev](https://github.com/TheoLeeCJ/SemIf-OpenJev); README and catalogs updated). The domains x.com, HN, Medium, Reddit, Substack, Discord, and Hugging Face were unreachable from this audit's network due to DNS pollution (their content was verified on 09-22 or is readable via mirrors); Zhihu 403s (owner-confirmed accessible in a browser, full text pending), Xiaohongshu “temporarily unavailable” redirects, a businesswire 403, and the WeChat verification wall all match existing entry annotations. No new dead links found. The README's master label was updated to 2026-09-24 (Asia/Shanghai) on this basis |
...
## September 26 incremental verification and exclusions

[TypeSafe Models](https://docs.typesafe.ai/models) still lists `jev-1.13.0`; both `jev-latest` and `jev-preview` point to it, with no preview build. The [status page](https://status.typesafe.ai/) displayed API and Console online, with the September 23 latency incidents and older ones resolved. Vercel's [model page](https://vercel.com/ai-gateway/models/jev) says promotional pricing ended September 25, yet still displayed Free on September 26. No account billing was tested, so neither continued free access nor a definite charge is asserted. The [integration announcement](https://vercel.com/changelog/typesafe-ai-jev-now-available-on-ai-gateway) still carries the older promotion copy.

- **First-party framework and platform material:** [Pydantic Jev model docs](https://pydantic.dev/docs/ai/models/typesafe/), [Evals example](https://pydantic.dev/articles/jev-evals), and [Gateway announcement](https://pydantic.dev/articles/jev-pydantic-ai-gateway); [Composio TypeSafe docs](https://docs.composio.dev/docs/providers/typesafe) and [changelog](https://docs.composio.dev/reference/changelog); [Rig's main repo](https://github.com/0xPlaygrounds/rig), which says experimental; [Ax's first-party site source](https://github.com/ax-llm/ax/blob/main/website/content/_index.md); [Vercel Connect](https://vercel.com/connect/jev) and [eve's evaluation guide](https://github.com/vercel/eve/blob/main/docs/guides/evaluate.md). Pydantic's `LLMJudge` / `GEval` support was confirmed from its published article, not inferred from an old issue.
- **Community repositories:** The [TypeSafeAI organization](https://github.com/TypeSafeAI) explicitly says it is unofficial. Its [playground](https://github.com/TypeSafeAI/typesafe-playground) separates mock from live calls; its [harness](https://github.com/TypeSafeAI/jev-harness) is mainly an offline contract with host-supplied live transport. [verdict](https://github.com/khimaros/verdict) describes itself as independent and wire-compatible. [TradingAgents](https://github.com/TauricResearch/TradingAgents) and its [v0.5.1 release](https://github.com/TauricResearch/TradingAgents/releases) confirm optional social-post filtering. [PyGPT](https://github.com/szczyglis-dev/py-gpt) documents the inline plugin and v2.8.31 addition. [Convex decision evals](https://github.com/get-convex/convex-evals) are separate from its coding evals. No volatile star counts were added for these entries.
- **Research and raw records:** The eight new arXiv originals are listed individually in [RESEARCH_EN.md](RESEARCH_EN.md); the 13 earlier IDs were reopened, but that historical count was not turned into a current global total. [Jev-Benchmark](https://github.com/ZF-Utokyo/Jev-Benchmark) files `benchmark/providers.py`, `benchmark/protocol.py`, `data/provenance.json`, and `docs/protocol.md` were read to check ContractNLI and the native Jev call. [RLCDAlignBench](https://github.com/sumleo/RLCDAlignBench) and its [gated dataset card](https://huggingface.co/datasets/sumleo/RLCDAlignBench) were checked for scale and labels. [JEV-Star](https://github.com/sc2musa/Jev_Star) states the small-sample and simultaneous-interface-change limits.
- **Not included or claimed:** The existing `reticlehq/reticle` entry still says planned, not shipped; `TypeSafeAI` is not presented as official; `verdict` is not presented as TypeSafe's model; the playground's changing example count was omitted; and `Jev in the Wild`'s 2,170 is a GitHub sample under the paper's method, not this catalog's verified-integration count. Long-tail aggregator leads without code or primary evidence were not promoted into the catalog.
...
- **09-24 papers addition:** the WeChat article was fully readable (direct HTML fetch), but it served only as a **lead**. The 13 papers were admitted after an [arXiv API `id_list` batch query](https://export.arxiv.org/api/query?id_list=2609.22753,2609.23136,2609.24052,2609.24965,2609.23986,2609.26532,2609.26550,2609.23959,2609.24395,2609.25845,2609.23886,2609.24574,2609.26758) confirmed each ID, title, author list, and first-submission date. The article's grouping (first applications / Jev-Anything / open alternatives and stress tests) and its “six papers on 09-21 alone” claim match the API response; the article named no authors, so author lists come from the API. Keyword site search may still miss items, so the 09-22 “no official paper found” conclusion is kept as-is.
